...
RoutePlanner::RoutePlanner(RouteModel &model, float start_x, float start_y, float end_x, float end_y): m_Model(model) {
    ...
    constexpr float PERCENT_CONVERSION = 0.01;
    start_x *= PERCENT_CONVERSION;
    start_y *= PERCENT_CONVERSION;
    end_x *= PERCENT_CONVERSION;
    end_y *= PERCENT_CONVERSION;
    ...
    //cout << "Distance between start node and end node is " << start_node->distance(*end_node) << endl;
}
...
RouteModel::Node *RoutePlanner::NextNode() {
    std::sort(open_list.begin(), open_list.end(),
        [] (RouteModel::Node *a, RouteModel::Node *b) {
            auto a_f = a->g_value + a->h_value;
            auto b_f = b->g_value + b->h_value;
            return a_f > b_f;
        }
    );
